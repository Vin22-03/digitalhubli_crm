...
export const createAdvisor = async (req, res) => {
  ...
  try {
    ...
    const [result] = await connection.query(
      `INSERT INTO \`User\`
       (name, email, phone, password, brandName, advisorUrl, role, isActive, mustChangePassword, createdAt, updatedAt)
       VALUES (?, ?, ?, ?, ?, ?, 'ADVISOR', 1, 0, NOW(), NOW())`,
  ...
  try {
    ...
    if (email) {
      const [emailRows] = await connection.query(
        ...
        [email.trim().toLowerCase(), numericAdvisorId]
      );
    ...
    await connection.query(
      `UPDATE \`User\`
       SET name = ?, email = ?, phone = ?, brandName = ?, advisorUrl = ?, updatedAt = NOW()
       WHERE id = ?`,
      [name, email ? email.trim().toLowerCase() : null, phone, brandName || null, finalAdvisorUrl, numericAdvisorId]
    );
