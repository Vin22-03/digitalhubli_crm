...
function AdminAdvisors() {
  ...
  const totalPages = Math.max(
    1,
    Math.ceil(filteredAdvisors.length / ITEMS_PER_PAGE)
  );
  ...
  const handleCreateOrEdit = async (e) => {
    ...
    try {
      ...
      if (mode === "edit" && selectedAdvisor) {
        await API.put(`/admin/advisors/${selectedAdvisor.id}`, {
          name: form.name.trim(),
          phone: form.phone.trim(),
      ...
      setErrorMsg(error?.response?.data?.message || "Failed to save advisor.");
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleToggleStatus = async (advisor) => {
    try {
      ...
      await API.patch(`/admin/advisors/${advisor.id}/toggle`);
      await fetchInitialData();
    } catch (error) {
    ...
    return (
      <div className="mt-6 flex flex-col gap-3 rounded-[24px] border border-blue-100 bg-white/75 px-4 py-4 shadow-[0_10px_25px_rgba(37,99,235,0.04)] sm:flex-row sm:items-center sm:justify-between">
        <p className="text-sm text-slate-600">
          ...
          to <span className="font-semibold text-slate-900">{endItem}</span>{" "}
          of{" "}
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="rounded-xl border border-blue-200 bg-white px-4 py-2 text-sm font-semibold text-slate-700 transition hover:bg-blue-50 disabled:cursor-not-allowed disabled:opacity-40"
          >
            Previous
          </button>

          <span className="rounded-xl border border-blue-100 bg-blue-50 px-4 py-2 text-sm font-bold text-blue-700">
      ...
      {showModal && (
        <div className="fixed inset-0 z-50">
          ...
          <div className="absolute inset-y-0 right-0 flex w-full justify-end overflow-hidden">
            <div className="h-full w-full max-w-[920px] border-l border-blue-100 bg-[linear-gradient(180deg,#ffffff,#f7fbff)] shadow-[-20px_0_60px_rgba(15,23,42,0.16)] xl:max-w-[980px]">
                  ...
                  {mode === "password" && (
                    ...
                    >
                      ...
                      <div className="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">
                        This will set a new password and force the advisor to
